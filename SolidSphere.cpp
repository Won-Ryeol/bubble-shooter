...
void SolidSphere::collisionHandling(SolidSphere& sph) {
	if (collisionDetection(sph) && sph.moved != true) {
		...
		float k = sqrt(3) / 2;
		if      (sph.center[0] < -11 * r) { sph.setCenter(center + Vector3(r, -2 * r * k, 0)); }  //left boundary
		else if (sph.center[0] > 11 * r)  { sph.setCenter(center + Vector3(-r, -2 * r * k, 0)); } //right boundary
...
void SolidSphere::setmarked() { marked = true; }
void SolidSphere::resetmarked() { marked = false; }
bool SolidSphere::getmarked() const { return marked; }
